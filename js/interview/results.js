(function (App) {
    ...
    App.downloadMarkdownReport = function () {
        if (!s.ratings.length) return;
        ...
        var md = [];
        md.push('# Interview Report');
        md.push('');
        md.push('| Field | Value |');
        md.push('|-------|-------|');
        md.push('| **Date** | ' + date + ' |');
        md.push('| **Interviewer** | ' + s.interviewerName + ' |');
        md.push('| **Candidate** | ' + s.intervieweeName + ' |');
        if (s.candidateLevel !== null && s.candidateLevel !== undefined && App.LEVEL_LABELS[s.candidateLevel]) {
            md.push('| **Exp. Level** | ' + App.LEVEL_LABELS[s.candidateLevel] + ' |');
        }
        ...
        if (sw.strengths.length > 0) {
            md.push('### Strengths');
            sw.strengths.forEach(function (t) {
                md.push('- **' + (App.TOPIC_LABELS[t.key] || t.key) + '** (' + t.avg.toFixed(1) + ')');
            });
            md.push('');
        }
        if (sw.weaknesses.length > 0) {
        ...
        md.push('## Topic Knowledge');
        md.push('');
        md.push('| Topic | Avg | Questions | Level |');
        md.push('|-------|-----|-----------|-------|');
        var sortedKeys = Object.keys(topicStats).sort(function (a, b) {
            return (topicStats[b].total / topicStats[b].count) - (topicStats[a].total / topicStats[a].count);
        });
        sortedKeys.forEach(function (key) {
            var t = topicStats[key];
            var tAvg = t.total / t.count;
            md.push('| ' + (App.TOPIC_LABELS[key] || key) + ' | ' + tAvg.toFixed(1) + ' | ' + t.count + ' | ' + App.LEVEL_LABELS[App.getLevelIndex(tAvg)] + ' |');
        });
        md.push('');

        // Notes
        if (s.introNotes) {
            md.push('## Introduction Notes');
            md.push('');
            md.push(s.introNotes);
            md.push('');
        }
        if (s.wrapupNotes) {
            md.push('## Wrap-up Notes');
            md.push('');
            md.push(s.wrapupNotes);
            md.push('');
        }

        // Questions
        md.push('## Question Breakdown');
        md.push('');
        s.sessionQuestions.forEach(function (q, i) {
            if (i >= s.ratings.length) return;
            var topicLabel = App.TOPIC_LABELS[q.topic] || q.topic;
            var rating = q.skipped ? '**Skipped**' : '**' + s.ratings[i] + '/5**';
            md.push('**#' + (i + 1) + '** ' + topicLabel + ' | ' + App.LEVEL_LABELS[q.level] + ' | ' + rating);
            md.push('> ' + q.question);
            if (q.code) {
                md.push('');
                md.push('```');
            ...
            if (q.notes && q.notes.trim()) {
                md.push('');
                md.push('*Notes: ' + q.notes.trim() + '*');
            }
            md.push('');
        });

        md.push('---');
        md.push('*Generated by [Interview Tool](https://khizanag.github.io/interview)*');
        ...
        var safeName = s.intervieweeName.replace(/[^a-zA-Z0-9]/g, '-').toLowerCase();
        var dateStr = new Date().toISOString().slice(0, 10);
        a.download = 'interview-' + safeName + '-' + dateStr + '.md';
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        URL.revokeObjectURL(url);
