        ...
        <section id="s-cicd" style={{ background: C.surface, borderTop: `1px solid ${C.border}`, borderBottom: `1px solid ${C.border}`, padding: "96px 48px" }}>
            <div style={{ maxWidth: 1200, margin: "0 auto" }}>
                <Reveal>
                    ...
                    <SectionHeading sub="The machinery behind every TBC UZ release. Bitrise → ASC → TestFlight → App Store. ~45 min end-to-end on a clean green build.">
                ...
                <div style={{ background: C.bg, border: `1px solid ${C.border}`, borderRadius: 16, padding: "40px 48px", marginBottom: 32 }}>
                    ...
                    <WorkflowStep n={2} total={8} title="PR opened → Bitrise triggers" color={P} delay={0.08}>
                    ...
                    <WorkflowStep n={4} total={8} title="Merge to master → TestFlight Internal workflow" color={C.yellow} delay={0.24}>
                    ...
                    <WorkflowStep n={8} total={8} title="Submit to App Store Review" color={C.accent} delay={0.56}>
                        Every 2–3 weeks. Release notes auto-generated from Jira tickets. Phased release enabled. Manual release gate for the final 100%.
                    </WorkflowStep>
                </div>
          ...
          inputs:
            ...
            - team_id: $TEAM_ID`}</CodeBlock>
                    ...
                    <div>
                        ...
                        <div style={{ marginTop: 16 }}>
                            <CalloutBox color={C.yellow} icon="⏱" label="DURATIONS IN PRODUCTION">
                                <b>PR build:</b> ~10 min · <b>Master TF Internal:</b> ~28 min · <b>Master → External TF:</b> +24h (Apple review) · <b>Full App Store release:</b> +48h median from submission to 100% phased rollout completion.
                            </CalloutBox>
                        </div>
                    </div>
                </div>
                ...
                <CalloutBox color={C.accent} icon="🎯" label="THE dev.config TRICK">
                    Each sub-repo (Feature-Cards, Core-Models, etc.) has its own branch. <b>dev.config</b> maps each repo to the branch RepoSync should check out for this build. This is how we ship features that span 10+ repos as one cohesive build — without a monorepo.
                </CalloutBox>
            </div>
        </section>
    );
