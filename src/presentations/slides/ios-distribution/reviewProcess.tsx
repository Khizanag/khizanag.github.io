        ...
        <section id="s-review" style={{ padding: "96px 48px" }}>
            <div style={{ maxWidth: 1200, margin: "0 auto" }}>
                <Reveal>
                    ...
                    <SectionHeading sub="The black box that decides whether your work reaches users. Here&rsquo;s what actually happens inside it.">
                ...
                <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 16, padding: "40px 48px", marginBottom: 32 }}>
                    <WorkflowStep n={1} total={5} title="Automated validation (minutes)" color={P} delay={0}>
                        Static analysis of the .ipa. Bundle ID, signing, entitlements, minimum OS version, app size, required device capabilities, Privacy Manifest completeness. If any of this fails, you&rsquo;re rejected before a human sees it.
                    </WorkflowStep>
                    ...
                    <WorkflowStep n={4} total={5} title="Guideline check (during functional)" color={C.red} delay={0.3}>
                        Reviewer matches what they see to App Review Guidelines 1.0&ndash;5.6. If a flag is raised, they attach the specific guideline number to the rejection.
                    </WorkflowStep>
                    <WorkflowStep n={5} total={5} title="Decision &amp; release" color={C.accent} delay={0.4}>
                        Approved &rarr; you get to release (manual, scheduled, or phased). Rejected &rarr; Resolution Center message with guideline citation. You can fix &amp; resubmit, or appeal to the App Review Board.
                    </WorkflowStep>
                </div>

                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 32 }}>
                    <CalloutBox color={C.yellow} icon="⚡" label="EXPEDITED REVIEW">
                        Apple grants ~1&ndash;2 expedited reviews per year per team for genuine emergencies &mdash; critical bug affecting many users, time-sensitive event. Abuse it and they stop granting. Request via the Contact Us form in ASC; expect a human response within ~4h.
                    </CalloutBox>

                    <CalloutBox color={C.red} icon="⚖" label="APP REVIEW BOARD">
                        If you disagree with a rejection, you can appeal to the App Review Board. In practice: respond politely in Resolution Center first. Appeals work best for guideline interpretation disputes, not &ldquo;I forgot to implement it.&rdquo;
                    </CalloutBox>
                </div>

                <CalloutBox color={P} icon="🎯" label="WHAT REVIEWERS ACTUALLY DO">
                    Public interviews with former App Reviewers describe a <b>queue-based system</b> where each reviewer gets ~50&ndash;100 apps per day, uses a standardized test device, has a scripted checklist, and can escalate edge cases to specialists (e.g., financial apps, health apps, kids apps). The &ldquo;automated&rdquo; pieces are pre-checks; humans do the actual judgement.
                </CalloutBox>
            </div>
        </section>
    );
