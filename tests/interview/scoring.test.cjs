...
const Scoring = require('../../js/interview/scoring.js');
...
describe('IRT Functions', function () {
    test('expectedOutcome: theta == b -> 0.5', function () {
        const p = Scoring.expectedOutcome(1.0, 1.0);
        assert(Math.abs(p - 0.5) < 0.001, 'Expected ~0.5 got ' + p);
    });

    test('expectedOutcome: theta >> b -> ~1.0, theta << b -> ~0.0', function () {
        const high = Scoring.expectedOutcome(5.0, 0.0);
        const low = Scoring.expectedOutcome(-5.0, 0.0);
    ...
    test('itemInformation: maximum at theta == b', function () {
        const atB = Scoring.itemInformation(1.0, 1.0);
        const away = Scoring.itemInformation(3.0, 1.0);
        assert(atB > away, 'Info at theta=b should be > info away from b');
    });

    test('itemInformation: equals a^2 * 0.25 at theta == b', function () {
        const info = Scoring.itemInformation(0, 0, 1.0);
...
describe('Engine', function () {
    test('createEngine: initial state', function () {
        const engine = Scoring.createEngine(['swift', 'swiftui']);
    ...
    test('High ratings converge theta upward', function () {
        const engine = Scoring.createEngine(['swift']);
        for (let i = 0; i < 5; i++) {
            Scoring.processRating(engine, { topic: 'swift', level: 2 + i }, 5);
        }
        const score = Scoring.thetaToScore(engine.theta);
    ...
    test('SE decreases with more questions', function () {
        ...
        const se1 = Scoring.standardError(engine);
        Scoring.processRating(engine, { topic: 'swift', level: 3 }, 3);
        const se2 = Scoring.standardError(engine);
    ...
    test('CI narrows with more questions', function () {
        const engine = Scoring.createEngine(['swift']);
        Scoring.processRating(engine, { topic: 'swift', level: 2 }, 3);
        const ci1 = Scoring.confidenceInterval(engine);
        const range1 = ci1.upper - ci1.lower;
        for (let i = 0; i < 5; i++) {
            Scoring.processRating(engine, { topic: 'swift', level: 2 }, 3);
        }
        const ci2 = Scoring.confidenceInterval(engine);
        const range2 = ci2.upper - ci2.lower;
        assert(range2 < range1, 'CI range should narrow: ' + range1 + ' -> ' + range2);
    });
});

describe('Adaptive Selection', function () {
    test('Topic coverage: underrepresented topic gets selected', function () {
        const engine = Scoring.createEngine(['swift', 'swiftui', 'concurrency']);
        ...
        const selected = Scoring.selectQuestion({
            pool: pool,
            engine: engine,
    ...
    test('Max info selection: picks question closest to current theta', function () {
        ...
        const selected = Scoring.selectQuestion({ pool: pool, engine: engine, topics: ['swift'] });
...
describe('Serialization', function () {
    test('Serialize/deserialize roundtrip preserves state', function () {
        const engine = Scoring.createEngine(['swift', 'concurrency']);
        Scoring.processRating(engine, { topic: 'swift', level: 3 }, 4);
        Scoring.processRating(engine, { topic: 'concurrency', level: 2 }, 2);

        const serialized = Scoring.serializeEngine(engine);
        const json = JSON.parse(JSON.stringify(serialized)); // Simulate localStorage
        const restored = Scoring.deserializeEngine(json);
...
describe('Migration', function () {
    test('migrateFromAvg: avg 4.5 -> high score, avg 1.5 -> low score', function () {
        const high = Scoring.migrateFromAvg(4.5, 10);
        const low = Scoring.migrateFromAvg(1.5, 10);
    ...
    test('migrateFromAvg: avg 3.0 -> middle score', function () {
        const mid = Scoring.migrateFromAvg(3.0, 10);
...
describe('Summary', function () {
    test('getSummary returns complete data', function () {
        ...
        const summary = Scoring.getSummary(engine);
